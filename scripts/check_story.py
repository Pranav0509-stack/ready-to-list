...
rao = items[items.id == "D0003"].iloc[0]
print("Rao D0003 slot", rao.slot_start, rao.slot_end)
record_outcome(conn, "J1", DEMO_DAY, "D0003", "effective")
purpose = plan_after("effective", "notice")
rec = next_date(conn, "D0003", "effective", purpose, DEMO_DAY)
print("next", purpose, rec)
confirm_next_date(conn, "D0003", rec, DEMO_DAY)
cid = items.id.iloc[0]
...
print(conn.execute("SELECT next_date,next_purpose FROM cases WHERE id='D0003'").fetchone()[:])
