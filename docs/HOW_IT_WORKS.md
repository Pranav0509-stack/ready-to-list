...
Judge. Today's list: the cause list for any sitting day with time, listing number, score, advocate and why the case is listed today; remove a case or restore; approve. Docket: every case with score, status, flags, age, listings and what happened; open a case for its factor points, last order and history. Priority: the five weights, adjustable with age never below 20, re-scoring the docket live. Calendar: the quarter with holidays, leave and each day's load. Results: the five case-study measures against today's rules, success by listing number, and the cumulative hearings that moved the judge's cases. How Samay decides: the model, the score and the day-building steps in full.
